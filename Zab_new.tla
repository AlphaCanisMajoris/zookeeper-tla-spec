------------------------------ MODULE Zab_new ------------------------------
(* This is the formal specification for the new Zab consensus algorithm.*)
...
CONSTANTS INITIAL, SYNC, RUNNING, SHUTDOWN
\* Message types
...
InitServerVars == /\ state         = [s \in Server |-> LOOKING]
                  ...
                  /\ servingState  = [s \in Server |-> INITIAL]
...
Restart(i) ==
        ...
        /\ \/ /\ IsLooking(i)
              /\ UNCHANGED <<state, zabState, servingState, learners, followerVars, msgVars,
...
LeaderProcessCEPOCH(i, j) ==
        ...
        /\ UNCHANGED <<state, zabState, servingState, currentEpoch, history, lastCommitted, learners, 
...
LeaderProcessACKEPOCH(i, j) ==
        ...
        /\ UNCHANGED <<state, servingState, acceptedEpoch, lastCommitted, learners, cepochRecv, ackldRecv, 
                sendCounter, followerVars, electionVars>>
        /\ UpdateRecorder(<<"LeaderProcessACKEPOCH", i, j>>)
-----------------------------------------------------------------------------    
(* Follower receives NEWLEADER. Update history. *)
           ...
           IN /\ infoOk
              /\ \/ \* 1. f.p not equals e', starts a new iteration.
                 ...
                 \/ \* 2. f.p equals e'.
                    ...
                    \*    IN Reply(i, j, m)
                    /\ servingState' = [ servingState EXCEPT ![i] = SYNC ]
...
(* Follower receives NEWLEADER. Update f.a. *)
FollowerProcessNEWLEADERAfterUpdatingHistory(i, j) ==
        /\ IsFollower(i)
        /\ PendingNEWLEADER(i, j)
        /\ servingState[i] = SYNC
        /\ currentEpoch' = [currentEpoch EXCEPT ![i] = acceptedEpoch[i]]
        /\ LET m == [ mtype |-> ACKLD,
                      mzxid |-> LastZxidOfHistory(history[i]) ]
           IN Reply(i, j, m)
        /\ UNCHANGED <<history, servingState, violatedInvariants>>
        /\ UNCHANGED <<followerVars, state, zabState, learners, cepochRecv,
                                    ackeRecv, ackldRecv>>
        /\ UNCHANGED <<acceptedEpoch, lastCommitted, sendCounter, electionVars, 
                proposalMsgsLog, epochLeader>>
        /\ UpdateRecorder(<<"FollowerProcessNEWLEADERAfterUpdatingHistory", i, j>>)
              ...
              /\ \/ \* 1. has not broadcast COMMITLD
                    ...
                    /\ \/ \* 1.1. ackldRecv becomes quorum,
                       ...
                       \/ \* 1.2. ackldRecv still not quorum.
                          /\ ~AckldRecvBecomeQuorum(i)
                          /\ Discard(j, i)
                          /\ UNCHANGED <<zabState, servingState, lastCommitted>>
...
LeaderProcessRequest(i) ==
        ...
        /\ UNCHANGED <<state, zabState, servingState, acceptedEpoch, currentEpoch, lastCommitted,
...
LeaderProcessACK(i, j) ==
        ...
        /\ UNCHANGED <<state, zabState, servingState, acceptedEpoch, currentEpoch, leaderVars,
                    followerVars, electionVars, proposalMsgsLog, epochLeader>>
                 ...
                 \/ /\ pending
                    /\ LET firstElement == history[i][lastCommitted[i].index + 1]
                       ...
                       \/ /\ match
                          ...
                          /\ UNCHANGED violatedInvariants
        /\ Discard(j, i)
        /\ UNCHANGED <<state, zabState, servingState, acceptedEpoch, currentEpoch, history,
                    leaderVars, followerVars, electionVars, proposalMsgsLog, epochLeader>>
...
Next ==
        ...
        \/ \E i, j \in Server: FollowerProcessNEWLEADERAfterUpdatingHistory(i, j)
